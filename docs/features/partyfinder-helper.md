...
- 更新机制：
  - 固定使用 GitHub 主仓库 `today080221/risingstones-partyfinder-helper` 和 Gitee 国内镜像 `jianwen1126/risingstones-partyfinder-helper` 检查最新 Release。
  - 加载前端时由本地代理做 GeoIP 检测：中国大陆或检测失败默认 Gitee，海外推荐 GitHub。
  - 支持启动时自动检查更新。
...
- `GET /api/update/check?provider=<github|gitee>&repo=<owner/repo>`
  读取 GitHub/Gitee 最新 Release，返回当前版本、最新版本、发布页和附件下载入口；远端 Release 不存在时回退读取最新 Git tag。

- `GET /api/geoip`
  通过公开 GeoIP 服务判断当前出口 IP 所在国家/地区，返回推荐下载节点；失败时回退到 Gitee。
...
- 更新检查：侧栏提供 GitHub/Gitee 更新源、仓库路径、启动时检查开关和检查结果卡片；服务端代理发布源 API，避免前端跨域限制。
