...
- 聚合检索与 NGA compact/cache 面板二次优化：`npm test` 127/127；`npm run build` 通过；`cargo check --manifest-path src-tauri/Cargo.toml` 通过；`cargo test --manifest-path src-tauri/Cargo.toml` 10/10；`npm run test:e2e` 9/9；parser curated harness 213/213。
