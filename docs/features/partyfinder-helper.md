...
- 聚合检索与 NGA compact 面板二次优化：`npm test` 119/119；`npm run build` 通过；`cargo check --manifest-path src-tauri/Cargo.toml` 通过；`cargo test --manifest-path src-tauri/Cargo.toml` 8/8；`npm run test:e2e` 9/9；parser curated harness 213/213。
