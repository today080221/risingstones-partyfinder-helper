...
- `risingstones_nga_collect_visible_samples`
  在用户当前打开的 NGA 网页窗口中按请求间隔读取已渲染帖子；当前页入口限定为已确认招募板或 `read.php?tid=...` 帖子详情。遇到 `misc/adpage_insert_2.html?...` 继续浏览页时，优先自动点击继续按钮，失败时再提示用户处理。帖子正文按“首楼 + 楼主有效补充”拼接，过滤顶帖和纯占位楼层。聚合默认按最近 14 天活跃主题打开正文，并接收压缩 cache 索引，优先读取新主题，再复核超过默认 12 小时、缺正文或状态不确定的已保存主题。
...
- NGA cache 优先体验：启动或前端刷新后立即展示本地已保存招募；桌面版默认自动复核待刷新项，聚合按钮旁展示新增、待刷新、已复核的轻量进度。
...
- Playwright e2e：当前 7 条通过，覆盖首屏渲染、NGA 高级/开发诊断折叠、地区多选、保持本机网页会话提示、NGA-only 聚合、official-only 聚合、石之家求职视图和本地标签筛选。
- 聚合检索与 NGA compact 面板二次优化：`npm test` 116/116；`npm run build` 通过；`cargo check --manifest-path src-tauri/Cargo.toml` 通过；`cargo test --manifest-path src-tauri/Cargo.toml` 8/8；`npm run test:e2e` 7/7；parser curated harness 213/213。
- NGA Parser 专项回测：本地样本池 396/396 条含正文， curated harness 30 条 fixture、213 条断言全通过；高置信有效解析 225 条，Wilson 95% 置信区间约 98.2% - 100.0%。
