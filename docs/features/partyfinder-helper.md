# 阿谢姆水晶（Azem's Crystal）

## Goal

构建一个名为“阿谢姆水晶（Azem's Crystal）”的本地网页/桌面工具，针对石之家国服副本招募页面，按选定副本全量拉取官方筛选结果，并在本地筛选符合用户要求的进度、攻略、时间和空缺职业。

## Confirmed Requirements

- 工具形态：本地网页工具，React + Vite + TypeScript 前端，Node/Express 本地代理。
- 全量范围：必须先选定 `fb_name`，再按官方分页完整拉取该副本的招募结果。
- 聚合入口：右上角“聚合检索”按左侧“结果来源”多选项处理数据源；默认全选石之家和 NGA。石之家仍要求先选副本，未选时只跳过石之家并提示，不阻断 NGA；旁边提供次级“强制刷新”动作。NGA 强制刷新只复核当前副本口径和当前地区板匹配的缓存帖，不带职业、时间、标签等本地细筛。
- 石之家缓存：按当前副本查询条件保存 15 分钟轻缓存；刷新或重进后先展示命中缓存，缓存过期后在统一读取锁内后台全量刷新；右上角“强制刷新”跳过缓存。
- 数据范围：普通模式只保留副本类型和副本名称；石之家按副本自带队伍类型取完整分页，不再暴露队伍构成、拉取位置或大区请求参数。
- NGA 副本范围筛选使用 Parser 主副本字段和石之家元数据做别名匹配；当前/最近期零式按石之家零式分类最顶 4 个副本解释 `M9S-M12S`、`M12S`、`零式1-4`、`零式4层` 等玩家写法。
- 本地筛选条件：
  - 标签/类型筛选，统一作用于石之家标签、NGA Parser tag 和结构化要求；多选默认要求全部命中，可用标题行的“任一命中”小按钮切换为 OR；石之家带“求职”标签的招募会归入“玩家求职”视图。
...
NGA 本地聚合仅在 Tauri 桌面运行时可用，Node/浏览器开发模式会显示不可用提示，不尝试跨域读取或绕过访问限制。

- `risingstones_nga_session_status`
  返回 NGA 本机网页窗口状态、保持本机网页会话选项、本机 WebView profile 路径和最近采集时间。

- `risingstones_nga_open_session`
  打开用户可见的 NGA WebView。默认读取公开可见页面；开启保持本机网页会话时使用本应用专属 WebView 数据目录，关闭时使用临时会话。聚合读取可用最小化窗口模式，手动打开仍正常显示。
...
- `risingstones_nga_collect_visible_samples`
  在用户当前打开的 NGA 网页窗口中按请求间隔读取已渲染帖子；当前页入口限定为已确认招募板或 `read.php?tid=...` 帖子详情。遇到 `misc/adpage_insert_2.html?...` 继续浏览页时，默认等待用户处理；只有用户主动开启“自动处理普通继续浏览页”后，才会在可见窗口中辅助点击明确的继续按钮。帖子正文按“首楼 + 楼主有效补充”拼接，过滤顶帖和纯占位楼层；同主题后续页会受限追读楼主有效补充。聚合默认先快扫活跃窗口 metadata，并接收压缩 cache 索引；只有新主题、标题或活跃时间变化、缺正文或超过默认 12 小时待复核的已保存主题才打开正文。
...
- 便携包构建：`scripts/build-portable.mjs` 组装 Windows x64 zip，包含一键启动 `阿谢姆水晶（Azem's Crystal）.exe`、`app/server.cjs`、`app/dist`、备用 `runtime/node.exe`、启动脚本、文档和许可文件。
...
- NGA 本地聚合基础版：Tauri 桌面版提供 NGA 本机网页窗口、保持本机网页会话风险提示、清除本机网页状态、可见可取消有限频样本采集、逐帖详情正文采集、已存样本正文补齐、1500 条本地去重样本池、样本分析报告、Parser v1，以及官方/NGA 统一来源卡片。
...
- NGA cache 优先体验：启动或前端刷新后立即展示本地已保存招募；默认只显示待刷新状态，不自动打开 NGA 窗口，聚合按钮旁展示快扫、新帖、复核、归档和清理的轻量进度。
- 石之家短缓存：当前副本查询结果写入 15 分钟本地轻缓存，页面刷新后优先命中缓存；过期后自动后台刷新，强制刷新会跳过缓存。
- NGA 公开页面快速读取已下线：不保留高级设置入口、前端 API 或 Tauri command；默认读取路径只走现有 WebView。NGA stale 自动刷新默认不自动弹出网页窗口。
- 统一读取锁：自动刷新、聚合检索、强制刷新和 NGA WebView 读取共用前端读取状态；读取中禁用聚合/强制刷新入口，保留停止按钮和轻量状态条。

## Verification Snapshot

> 以下是历史验证快照，不代表当前未提交工作区状态。本轮石之家 cache / NGA 实验读取收口的最新验证以 `docs/worklog/2026-05-10-official-cache-nga-public-fetch.md` 的 Validation 节为准。
...
- Playwright e2e：当前 10 条通过，覆盖首屏渲染、NGA 高级/开发诊断折叠、地区多选、数据来源面板折叠、标签/类型 4 行预览和“任一命中”小开关默认态、保持本机网页会话提示、NGA-only 聚合、official-only 聚合、石之家求职视图和本地标签筛选。
- 聚合检索与 NGA compact/cache 面板二次优化：`npm test` 129/129；`npm run build` 通过；`cargo check --manifest-path src-tauri/Cargo.toml` 通过；`cargo test --manifest-path src-tauri/Cargo.toml` 10/10；`npm run test:e2e` 9/9；parser curated harness 213/213。
- NGA Parser 专项回测：本地样本池 507/507 条含正文， curated harness 30 条 fixture、213 条断言全通过；高置信有效解析 264 条，Wilson 95% 置信区间约 98.2% - 100.0%。默认 `npm run validate:nga-parser` 只用 curated harness 作为失败门槛，本机样本池数量差异仅输出 warning；严格本机基线检查使用 `npm run validate:nga-parser:local`。
