...
- 数据范围：保留副本类型/名称、大区、队伍构成、位置、团队 A/B/C；石之家默认不带标签参数，尽量先取全量再交给本地筛选。
- 本地筛选条件：
...
- `risingstones_nga_collect_visible_samples`
  在用户当前打开的 NGA 网页窗口中按请求间隔读取已渲染帖子；当前页入口限定为已确认招募板或 `read.php?tid=...` 帖子详情。遇到 `misc/adpage_insert_2.html?...` 继续浏览页时，前端会等待用户点继续后自动重试。帖子正文按“首楼 + 楼主有效补充”拼接，过滤顶帖和纯占位楼层，并只返回 `title`、`body`、`url`、`author`、`publishedAt`、`forumId`、`topicId` 白名单字段。
...
- 聚合检索与 NGA compact 面板二次优化：`npm test` 106/106；`npm run build` 通过；`cargo check --manifest-path src-tauri/Cargo.toml` 通过；`npm run test:e2e` 7/7；parser harness 213/213。
