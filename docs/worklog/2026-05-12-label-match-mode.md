...
- 默认语义从 OR 改为 AND，会减少多标签筛选结果数量；需要通过 UI 文案让用户知道可以点亮“任一命中”恢复旧行为。
...
- `matchesLabelFilter` 改为按已选标签逐组匹配：默认 `every`，点亮“任一命中”后使用 `some`。
...
- `npm run desktop:dev`：通过，桌面窗口 `阿谢姆水晶 (Azem's Crystal)` 成功启动；测试后已停止 dev 进程。

## Final Status

- 分支：`codex/label-match-mode`
- 交付前同步：已再次执行 `git fetch origin`；分支已推送到 `origin/codex/label-match-mode`。
