...
release/阿谢姆水晶（Azem's Crystal）-v1.0.3-win-x64.zip.sha256
```

## 发布包内容

```text
阿谢姆水晶（Azem's Crystal）-v1.0.3-win-x64/
  阿谢姆水晶（Azem's Crystal）.exe 一键启动入口
  app/
    dist/               已构建前端
    server.cjs          已打包本地代理
  runtime/
...
```powershell
Get-FileHash '.\阿谢姆水晶（Azem''s Crystal）-v1.0.3-win-x64.zip' -Algorithm SHA256
```
...
- 根目录 `阿谢姆水晶（Azem's Crystal）.exe`：通过 Node.js SEA 注入启动脚本的一键入口。
...
- 解压 zip 后双击 `阿谢姆水晶（Azem's Crystal）.exe` 能打开本地页面。
